...
export default function CosmoScan() {
  ...
  const calculateScore = async () => {
    ...
    try {
      ...
      const alreadyLiked = await isFavorite("Scanned Product");
  ...
  async function handleFavorite() {
    if (liked) {
      ...
      const id = favs.find((f) => f.productName === "Scanned Product")?.id;
      if (id) await removeFavorite(id);
      setLiked(false);
    } else {
      await saveFavorite({ type: "photo", productName: "Scanned Product", ingredients: ingredients.join(", ") });
      setLiked(true);
    }
  }
