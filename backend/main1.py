...
def preprocess(img: np.ndarray) -> np.ndarray:
    """Tesseract-optimised preprocessing pipeline."""
    # Resize — critical for Tesseract accuracy
...
def _run_tesseract(img: np.ndarray) -> str:
    """Run Tesseract with the best general config (OEM 3, PSM 6)."""
    config = "--oem 3 --psm 6"
...
def run_ocr(image_bytes: bytes) -> dict:
    ...
    # Rotation correction then Tesseract preprocessing
    img_rot  = correct_rotation(img_bgr)
    img_prep = preprocess(img_rot)

    # Run Tesseract OCR
    full_text = _run_tesseract(img_prep).strip()

    # Confidence estimation via Tesseract data (mean of confident words)
    try:
        data     = pytesseract.image_to_data(img_prep, config="--oem 3 --psm 6",
                                             lang="eng",
                                             output_type=pytesseract.Output.DICT)
        confs    = [int(c) for c in data["conf"] if str(c).lstrip("-").isdigit() and int(c) >= 0]
        avg_conf = float(np.mean(confs)) / 100.0 if confs else 0.0
    except Exception:
        avg_conf = 0.8  # fallback — Tesseract typically reliable
...
async def analyze_image(file: UploadFile = File(...)):
    ...
    # Pass full OCR text directly to NER
    ingredients = run_ner(ocr_result["full_text"])
